...
{
	ft_memcpy(&icmp_reply->iphdr, buffer, IP_HDR_SIZE);
	ft_memcpy(&icmp_reply->icmphdr, buffer + IP_HDR_SIZE, ICMP_HDR_SIZE);
	ft_memcpy(&icmp_reply->original_iphdr, \
		buffer + IP_HDR_SIZE + ICMP_HDR_SIZE, IP_HDR_SIZE);
	ft_memcpy(&icmp_reply->original_udp_pckt, buffer + ICMP_HDR_SIZE + 2 * IP_HDR_SIZE, UDP_PCKT_SIZE);
}
